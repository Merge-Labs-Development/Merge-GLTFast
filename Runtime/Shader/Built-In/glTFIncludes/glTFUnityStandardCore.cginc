...
float3 NormalizePerVertexNormal (float3 n) // takes float to avoid overflow
{
    #if (SHADER_TARGET < 30) || UNITY_STANDARD_SIMPLE
        float3 normalized = normalize(n);
        normalized = max(normalized, 1e-5);
        return normalized;
    #else
...
UnityLight AdditiveLight (float3 lightDir, half atten)
{
    UnityLight l;
...
// deprecated
float3 WorldNormal(float4 tan2world[3])
{
...
// deprecated
    ...
    float3x3 ExtractTangentToWorldPerPixel(float4 tan2world[3])
    {
        float3 t = tan2world[0].xyz;
        float3 b = tan2world[1].xyz;
        float3 n = tan2world[2].xyz;
...
float3 PerPixelWorldNormal(float2 i_tex, float4 tangentToWorld[3])
{
...
#ifdef _PARALLAXMAP
    #define IN_VIEWDIR4PARALLAX(i) NormalizePerPixelNormal(float3(i.tangentToWorldAndPackedData[0].w,i.tangentToWorldAndPackedData[1].w,i.tangentToWorldAndPackedData[2].w))
    #define IN_VIEWDIR4PARALLAX_FWDADD(i) NormalizePerPixelNormal(i.viewDirForParallax.xyz)
#else
    #define IN_VIEWDIR4PARALLAX(i) float3(0,0,0)
    #define IN_VIEWDIR4PARALLAX_FWDADD(i) float3(0,0,0)
#endif
    ...
    #define IN_WORLDPOS_FWDADD(i) float3(0,0,0)
#endif

#define IN_LIGHTDIR_FWDADD(i) float3(i.tangentToWorldAndLightDir[0].w, i.tangentToWorldAndLightDir[1].w, i.tangentToWorldAndLightDir[2].w)
...
{
#ifdef _SPECGLOSSMAP
    float4 specGloss = SpecularGloss(i_texSpecGloss);
#else
    ...
    float3 diffColor = EnergyConservationBetweenDiffuseAndSpecular (Albedo(i_tex)*v_color, specColor, /*out*/ oneMinusReflectivity);
...
inline FragmentCommonData RoughnessSetup(float4 i_tex, float3 v_color)
{
    ...
    half oneMinusReflectivity;
    float3 specColor;
    float3 diffColor = DiffuseAndSpecularFromMetallic(Albedo(i_tex)*v_color, metallic, /*out*/ specColor, /*out*/ oneMinusReflectivity);
...
inline UnityGI FragmentGI (FragmentCommonData s, half occlusion, float4 i_ambientOrLightmapUV, half atten, UnityLight light, bool reflections)
{
    UnityGIInput d;
    d.light = light;
...
inline UnityGI FragmentGI (FragmentCommonData s, half occlusion, float4 i_ambientOrLightmapUV, half atten, UnityLight light)
{
...
//-------------------------------------------------------------------------------------
float4 OutputForward (float4 output, half alphaFromSurface)
{
...
float4 fragForwardBaseInternal (VertexOutputForwardBase i)
{
...
float4 fragForwardBase (VertexOutputForwardBase i) : SV_Target   // backward compatibility (this used to be the fragment entry function)
{
...
float4 fragForwardAddInternal (VertexOutputForwardAdd i)
{
    UNITY_APPLY_DITHER_CROSSFADE(i.pos.xy);
    ...
    UNITY_EXTRACT_FOG_FROM_EYE_VEC(i);
    UNITY_APPLY_FOG_COLOR(_unity_fogCoord, c.rgb, float4(0,0,0,0)); // fog towards black in additive pass
    return OutputForward (c, s.alpha);
}

float4 fragForwardAdd (VertexOutputForwardAdd i) : SV_Target     // backward compatibility (this used to be the fragment entry function)
{
...
inline UnityGI FragmentGI(
    float3 posWorld,
    half occlusion, float4 i_ambientOrLightmapUV, half atten, half smoothness, float3 normalWorld, float3 eyeVec,
    UnityLight light,
    bool reflections)
{
